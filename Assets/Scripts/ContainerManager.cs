...
public class ContainerManager : MonoBehaviour {
	...
	private static void LoadImageBehavior(string currentModule, string elementName, int modifyState) {
		if(string.IsNullOrWhiteSpace(currentModule) || ! currentModule.Equals(GlobalData.CurrentModule)) return;
		Transform displayObject = GlobalData.CurrentDisplayObjectDic[elementName];
		if(! displayObject) return;
		Image image = displayObject.GetComponent<Image>();
		if(! image) return;
		string displayKey = $"{currentModule}_{elementName}";
		string imageUrl;
		GlobalData.DisplayObjectPathDic.TryGetValue(displayKey, out imageUrl);
		Material material = null;
		if(imageUrl != null) MaterialDic.TryGetValue(imageUrl, out material);
		if(material != null) {
		...
		if(modifyState != 0) GlobalData.ModifyCount += modifyState;
	}
	...
	public Transform AddDisplayObject(string imageUrl, Vector2 pos, Vector2 size, string elementName = null) {
		...
		if(! string.IsNullOrEmpty(imageUrl)) {
			if(MaterialDic.ContainsKey(imageUrl)) {
				...
				material = new Material(GlobalData.DefaultShader) {mainTexture = texture2};
...
//				GlobalData.DisplayObjectPathDic.Remove(displayObjectKey);
	...
	private static void RemoveAllModules() {
		...
		foreach(var pair in GlobalData.ModuleDic) pair.Value.Clear();
